...
from src.quantum.cpake import generate_kyber_keys, encrypt, decrypt
from src.quantum.params import KYBER_512SK_BYTES, KYBER_768SK_BYTES, KYBER_1024SK_BYTES, KYBER_SYM_BYTES, KYBER_SS_BYTES, \
...
def kem_encaps768(pubkey, message: str, seed=None):
    """
    ...
    params_k = 3
    b_mess = bytes(message,"utf-8")
    md = SHA3_256.new(b_mess)
    md.update(bytearray(seed))
    Hm = md.digest()
    Hm = [ cast_to_byte(x) for x in Hm ]
